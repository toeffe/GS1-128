/**
 * Pure GS1-128 logic — no DOM. Used by index.html and Node tests.
 */
(function (root, factory) {
  ...
  function isValidCalendarDate(yy, mm, dd) {
  ...
  function parseDate(s) {
    ...
    if (!isValidCalendarDate(yy, mm, dd)) return null;
    const year = yyToFullYear(yy);
    return dd === 0
  ...
  function parseDateObj(s) {
    ...
    const yy = parseInt(s.slice(0, 2), 10);
    const mm = parseInt(s.slice(2, 4), 10);
    const ddRaw = parseInt(s.slice(4, 6), 10);
    if (!isValidCalendarDate(yy, mm, ddRaw)) return null;
    const dd = ddRaw || 1;
  ...
  function composeSSCC(extension, prefix, serialNum) {
    ...
    if (!p) return { error: 'Company prefix is required.' };
    if (p.length > 15) return { error: 'Company prefix derived from GTIN is too long.' };
    const { serialLen, maxSerial } = getSsccSerialCapacity(p);
    if (serialLen < 1) return { error: 'Company prefix leaves no room for a serial number.' };
    if (serialNum > maxSerial) {
      return {
        error: `Serial counter exhausted for this prefix (max ${maxSerial}). A ${p.length}-digit company prefix only allows ${serialLen}-digit serial references (0–${maxSerial}). Lower sscc_current_sequence in sscc-prefix.json.`,
      };
    }
  ...
  function getSsccSerialCapacity(prefix) {
    const p = String(prefix || '').replace(/\D/g, '');
    if (!p || p.length > 15) return { serialLen: 0, maxSerial: 0 };
    const serialLen = 16 - p.length;
    if (serialLen < 1) return { serialLen: 0, maxSerial: 0 };
    return { serialLen, maxSerial: Math.pow(10, serialLen) - 1 };
  }
  ...
  function normalizeSsccPrefixEntry(item) {
    const seq = parseInt(item.sscc_current_sequence, 10);
    return {
      id: String(item.id || '').trim(),
      name: String(item.name || '').trim(),
      sscc_current_sequence: Number.isFinite(seq) && seq >= 0 ? seq : 0,
    };
  }

  function mergeSsccPrefixConfig(fileData, localData) {
    const byId = new Map();
    [...(localData?.prefixes || []), ...(fileData?.prefixes || [])].forEach(item => {
      if (!item?.id) return;
      const next = normalizeSsccPrefixEntry(item);
      const prev = byId.get(next.id);
      if (!prev || next.sscc_current_sequence > prev.sscc_current_sequence) byId.set(next.id, next);
    });
    const filePrefix = String(fileData?.company_prefix || '').replace(/\D/g, '');
    const localPrefix = String(localData?.company_prefix || '').replace(/\D/g, '');
    return {
      version: 1,
      company_prefix: filePrefix || localPrefix || '',
      prefixes: [...byId.values()].sort((a, b) =>
        (a.name || '').localeCompare(b.name || '', undefined, { sensitivity: 'base' }),
      ),
    };
  }
  ...
  function splitVerifyInput(raw) {
    ...
    const markers = [...text.matchAll(/\((?:00|01|02|10|11|15|17|37|3102)\)/g)];
  ...
  function parseFieldsWithStatus(raw) {
    ...
    if (skipped > 0) {
      warnings.push({
        sev: 'warn',
        msg: `Skipped ${skipped} unrecognized character${skipped === 1 ? '' : 's'} while parsing`,
      });
    }
  ...
  function parseFields(raw) {
    return parseFieldsWithStatus(raw).fields;
  }

  function barcodeTitleFromFields(fields) {
    if (!fields.length) return 'Barcode';
    if (fields[0].ai === '01' || fields[0].ai === '02') return 'Product';
  ...
  return {
    ...
    composeSSCC,
    getSsccSerialCapacity,
    mergeSSCCLogData,
    mergeItemsCatalog,
    mergeSsccPrefixConfig,
    normalizeSavedItem,
    normalizeSsccPrefixEntry,
    splitVerifyInput,
    normalise,
    findAI,
    formatValue,
