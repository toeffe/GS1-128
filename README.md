...
**Scan from photo** — on the Verify tab, use **Upload photo** (desktop) or **Choose photo** (mobile), or drag-and-drop on desktop, to decode CODE128 barcodes (JPEG, PNG, WebP, GIF, BMP, AVIF, TIFF). The photo scans automatically after selection. On mobile, **Choose photo** opens the system picker where you can pick from the gallery or take a new photo. Works offline when you open `index.html` directly; no local server. Chrome or Edge recommended on mobile. HEIC/HEIF (common on iPhone) is not supported — use JPEG or PNG. For best results, crop the photo to a single barcode; full labels with three barcodes may need several tries or better lighting.
